...
const ModalEvent = ({ idAttended, setIsOpen }: Props) => {
  ...
  const getAttended = async () => {
    const res = await axios.get(`http://localhost:3000/api/v1/attendeds/${idAttended}`)
    setAttended(res.data);
    setIsChecked(res.data.isAttended)
  }

  const updateAttended = async () => {
    const res = await axios.patch(`http://localhost:3000/api/v1/attendeds/${idAttended}`, { isAttended: isChecked })
    console.log(res);
    SuccessAlert("Asistencia actualizada correctamente")
  }
      ...
      <div className="relative p-4 w-full max-w-2xl max-h-full">
        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
          <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white">{attended?.date}</h3>
          ...
          <div className="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
            <button onClick={handleUpdate} data-modal-hide="default-modal" type="button" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Aceptar</button>
